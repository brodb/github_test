...
st.dataframe(df_racer.style
             .highlight_max(color='green',axis=0,subset=['Speed','Acceleration','Weight'])
             .highlight_min(color='red',axis=0,subset=['Speed','Acceleration','Weight'])
)

st.line_chart(df_racer, x='Speed', y=['Weight','Acceleration','Handling','Traction/Grip','Mini-Turbo'])

st.header('Racer Speed does not seem to correlate with number of races a character has won')
...
with right_column_1:
    ...
    df_best_racers = df_racer[['Character','Times First Place','Total Races']]
    # st.dataframe(df_best_racers)
    df_best_racers['Win Percentage'] = df_best_racers['Times First Place']/ df_best_racers['Total Races'] * 100
    df_best_racers = df_best_racers[['Character','Win Percentage']].sort_values('Win Percentage', ascending=False).iloc[0:x]
    st.dataframe(df_best_racers)
